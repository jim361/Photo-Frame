...
- **경계 표시 + 드래그 배치**: 줌바의 "경계"를 켜면 각 요소의 실제 영역이 보이고, 박스를 끌어 위치를 정합니다. 다른 요소와 수평/수직이 맞으면 가이드라인 스냅, 더블클릭으로 초기화
...
4. **다듬기** — "표시 · 레이아웃 설정"에서 세로/가로 탭별로 조절하거나, 미리보기에서 "경계"를 켜고 직접 드래그.
