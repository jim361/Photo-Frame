...
- 로고·서명: 이미지(투명 PNG) 또는 직접 입력한 텍스트 서명을 프레임에 각인. 등록 목록은 테마와 무관한 체크 배경으로 표시하며 로고의 원래 색은 유지
...
- 제거된 스퀘어·와이드로 저장한 작업·프리셋은 배치 값을 인스탁스로 이어받고 촬영 정보·출력 설정을 유지. 프레임 모양은 현재 인스탁스를 사용
...
의견이나 오류는 톱니바퀴 설정의 `의견·버그 보내기` 또는 [GitHub 피드백](https://github.com/jim361/Photo-Frame/issues/new/choose)에서 남길 수 있습니다. 작성에는 GitHub 로그인이 필요합니다. `제작자 GitHub`도 설정 안에 있으며, 링크를 직접 누를 때만 새 탭을 엽니다. 사진이나 편집 내용은 전달하지 않습니다.
...
|---|---|---|
| 프레임 합성·PNG/JPEG | 표준 Canvas·Blob·download | Chrome 152: 8종 × 세로/가로, 실제 인코딩. 다운로드 폴백은 이전 인앱 검증 |
...
기존 Playwright 개발 환경이 있으면 `node tools/check-browser.mjs`로 신선 저장소 초기화·콘솔 오류·외부 요청, 기존 프레임 픽셀 비교(인스탁스 흰색 변경 제외), 8종 세로/가로, 색·프리셋·출력·360/390/768px 화면을 확인할 수 있습니다. 2026-09-13 Chrome 152.0.7977.83에서 제거된 인스탁스 규격의 저장값 이전 검사까지 통과했습니다. `PLAYWRIGHT_MODULE`은 설치된 패키지 경로, `CHROME_PATH`는 로컬 Chrome 실행 파일로 지정할 수 있습니다. 앱의 런타임 의존성은 추가하지 않습니다. 스크린샷은 기본 임시 폴더 또는 `PHOTOFRAME_QA_DIR`에 기록합니다.
