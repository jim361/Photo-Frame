...
- 프레임 선택 버튼 10개에는 현재 사진의 작은 Canvas 썸네일과 텍스트 이름을 함께 표시한다. 인스탁스는 미니·스퀘어·와이드로 구분한다.
...
- 패널 폭·UI 배율에 따라 실제 내부 너비가 280px 이하가 되면 container query로 입력을 한 열, 프레임 스타일을 2열로 배치한다. 화면이 넓어도 좁힌 패널에서 필수 버튼이 잘리지 않는다.
...
- 인스탁스는 사진 비율을 보존하며 미니·스퀘어·와이드에서 따온 여백만 적용한다. 실제 필름의 고정 비율이나 인쇄 크기를 강제하지 않는다. 미니의 기존 가로 우측 여백은 유지하고 스퀘어·와이드는 항상 아래쪽에 정보를 둔다. 수치와 근거는 [프레임 렌더링](features/01-frame-rendering.md)을 따른다.
