...
- 내보내기는 스트립 순서의 설정·파일명 스냅샷으로 사전 확인한 뒤 순차 처리한다. 취소는 다음 사진부터, 재시도는 실패 작업만 같은 설정으로 실행한다.
...
`node tools/check.mjs`는 Node 표준 라이브러리로 구문·DOM·공통 렌더·외부 요청 코드·호환 표식·legacy 해시와 실제 복구/저장/내보내기·EXIF·핀치/정밀 이동·색상 분기를 검사한다. GitHub Pages 배포 전에도 같은 검사를 실행한다. 선택적인 `tools/check-browser.mjs`는 기존 Playwright 개발 환경에서 신선 저장소 초기화·콘솔·구버전 픽셀 일치·프로젝트 복원·실제 인코딩·반응형 화면을 확인한다. 브라우저 검증은 신선 컨텍스트와 `?v=` 캐시 우회를 사용한다.
