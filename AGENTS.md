...
최근 완료: 요소 개별 배치(구분자 제거·드래그), 한/영 글꼴 분리와 로컬 PC·파일 글꼴, 프레임 크기 핸들(모서리 그립·호버 강조·경계 기본 표시), 스타일·방향별 레이아웃 프리셋, 가로 인스탁스 회전 요소 드래그, 넘침 빨간 경고, 사진 원본 없는 프로젝트 JSON 저장·복원, 첫 화면 안내문 줄바꿈 개선.
