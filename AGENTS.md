...
- 사용자 요청에 따른 간단한 수정은 검증 후 별도 확인 없이 커밋·푸시와 배포 확인까지 진행한다.
...
- `render(src, opts, info)` — 원본 해상도로 합성 후 축소. `info.collect`가 참이면 요소 박스를 `info.boxes`로 수집. 스타일: `film` / `instax`(미니)/`instax-square`/`instax-wide` / `bottom`/`top` / `matte`/`gallery`/`keyline`/`minimal`(여백 분기 공유). 인스탁스 3종은 `instaxMetrics()`와 기존 렌더 분기를 공유하며 사진 비율을 유지한다. 기존 `instax`의 가로 우측 여백은 보존하고 스퀘어·와이드는 항상 하단 여백을 쓴다. 미리보기·현재 사진 썸네일·넘침 검사·내보내기가 같은 경로 사용
...
- 내보내기: 전체/현재 장·사전 검사·사진 사이 취소, 비율 패딩(`padToRatio`), File System Access API 폴더 저장(Chromium) + 다운로드 폴백, 제목 순번 명명. 새 설정은 원본 비율·프레임 색 패딩·2048px이며 이전 자동 비율·패딩색·크기 저장값은 숨긴 호환 입력으로 유지
...
추가 완료: 부분 프로젝트 저장·dirty 복구, 인스탁스 미니·스퀘어·와이드를 포함한 10종 프레임·색상·사진 썸네일, 부분 EXIF 채택·개별 노출 표시·날짜 형식, 전체/현재 장 내보내기·사전 검사·취소, 로컬 디자인 프리셋, 좁은 화면 고정 미리보기·핀치·정밀 이동. 미리보기 상단 구분선·소개와 내장 예제, 특정 제목·서명 예시, 선택 내보내기·실패 재시도 버튼은 제거했다. 인스탁스 종이는 흰색이며 EXIF 채택 후 해당 사진 범위로 전환한다. `node tools/check.mjs`는 표준 라이브러리 동작 검사까지 실행한다. 이전 Chrome 브라우저 검사와 24MP 합성 JPEG 1/6/12장 측정은 README의 날짜·조건 참조.
